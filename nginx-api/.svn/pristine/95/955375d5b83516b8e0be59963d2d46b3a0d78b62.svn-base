 ...
 * 
 */
public class MongoRefreshOperation extends MongoOperation {
	...
	public void execute(MongoDatabaseConnection connection, IDataSet dataSet)
		...
		while (iterator.next()) {
			...
			for (int i = 0; i < table.getRowCount(); i++) {
				...
				Object primaryValue = table.getValue(i,
						primaryColumn.getColumnName());
				if (primaryValue == null) {
					// FIXME logging
					continue;
				}
				collection.update(new BasicDBObject("_id", primaryValue),
						dbObject, true, false);
			}
		}
	}
